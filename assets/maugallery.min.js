...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));var modalElement=document.getElementById(lightboxId);if(modalElement){var bsModal=bootstrap.Modal.getOrCreateInstance(modalElement);bsModal.show()}},prevImage(){let activeImage=null;$("img.gallery-item").each(function(){if($(this)[0].src===$(".lightboxImage")[0].src){activeImage=$(this)}});let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).children("img").length){imagesCollection.push($(this).children("img"))}})}else{$(".item-column").each(function(){if($(this).children("img").data("gallery-tag")===activeTag){imagesCollection.push($(this).children("img"))}})}
let index=0;$(imagesCollection).each(function(i){if($(activeImage)[0].src===$(this)[0].src){index=i}});let prevIndex=(index-1+imagesCollection.length)%imagesCollection.length;let prev=imagesCollection[prevIndex]||imagesCollection[0];$(".lightboxImage").attr("src",$(prev).attr("src"))},nextImage(){let activeImage=null;$("img.gallery-item").each(function(){if($(this)[0].src===$(".lightboxImage")[0].src){activeImage=$(this)}});let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).children("img").length){imagesCollection.push($(this).children("img"))}})}else{$(".item-column").each(function(){if($(this).children("img").data("gallery-tag")===activeTag){imagesCollection.push($(this).children("img"))}})}
let index=0;$(imagesCollection).each(function(i){if($(activeImage)[0].src===$(this)[0].src){index=i}});let nextIndex=(index+1)%imagesCollection.length;let next=imagesCollection[nextIndex]||imagesCollection[0];$(".lightboxImage").attr("src",$(next).attr("src"))},createLightBox(gallery,lightboxId,navigation){$("body").append(`<div class="modal fade" id="${
